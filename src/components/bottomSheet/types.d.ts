...
interface BottomSheetProps {
  ...
  /**
   * Array of snap points that the bottom sheet will snap to. Can be numbers (pixels) or strings (percentages).
   * When provided, this takes precedence over the `height` prop.
   * 
   * `Default: undefined`
   * @type {(number | string)[]}
   * @example
  ...
  /**
   * Initial snap point index. Defaults to the first snap point (index 0).
  ...
  /**
   * Callback function that is called when the snap point changes.
   * 
   * @type {(index: number, position: number) => void}
   * @default undefined
   * @example
   * ```tsx
   * <BottomSheet
   *   onChange={(index, position) => {
   *     console.log('Snapped to index:', index, 'at position:', position);
   *   }}
   * />
   * ```
   */
